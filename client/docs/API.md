...
## 🚀 Getting Started (Cara Connect ke Backend)

Aplikasi client ini membutuhkan URL backend yang valid untuk berfungsi.

### Konfigurasi `.env`

Buat atau edit file `.env` (atau `.env.local` untuk development) di root folder `client`:

```bash
# File: .env.local atau .env.production
VITE_API_URL=https://tahfidz-bootcamp-api.adzan.workers.dev
```

### Note pada Production
Secara default, aplikasi akan mencoba menggunakan `VITE_API_URL`. Jika tidak ada, akan fallback ke URL production `https://tahfidz-bootcamp-api.adzan.workers.dev`.
...
Verifikasi OTP setelah registrasi untuk mengaktifkan akun.
...
_API Documentation untuk Tahfidz Bootcamp API v1.2.0_
