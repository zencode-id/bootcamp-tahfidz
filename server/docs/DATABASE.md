...
| 2   | `otp_codes`         | Kode OTP untuk verifikasi login           | 6            |
| 3   | `classes`           | Data kelas/halaqah                        | 7            |
| 4   | `class_members`     | Relasi santri-kelas                       | 6            |
| 5   | `student_profiles`  | Profil detail santri (foto, dll)          | 6            |
| 6   | `teacher_profiles`  | Profil detail guru tahfidz                | 6            |
| 7   | `class_transfers`   | Catatan perpindahan kelas                 | 10           |
| 8   | `surahs`            | Data referensi 114 surah Al-Quran         | 6            |
| 9   | `attendance`        | Catatan kehadiran                         | 13           |
| 10  | `memorization_logs` | Catatan hafalan (ziyadah/murojaah)        | 14           |
| 11  | `assessments`       | Penilaian hafalan harian                  | 11           |
| 12  | `exams`             | Data ujian tahfidz                        | 17           |
| 13  | `exam_results`      | Hasil ujian per santri                    | 18           |
| 14  | `reports`           | Raport semester santri                    | 28           |
| 15. | `sync_logs`         | Log sinkronisasi dengan Google Sheets     | 9            |
| 16. | `school_profiles`   | Profil sekolah/lembaga                    | 9            |
...
### 2. OTP Codes (Kode Verifikasi)

Menyimpan kode OTP untuk verifikasi login.
...
| `expires_at` | INTEGER  | NOT NULL                    | Waktu kadaluarsa (epoch ms) |
| `created_at` | DATETIME | DEFAULT now()               | Waktu dibuat                |
| `updated_at` | DATETIME | DEFAULT now()               | Waktu diperbarui            |

**Relasi:**

- Many-to-One: Ke `users` (cascade delete)

**Catatan:** OTP berlaku selama 5 menit (300000 ms).

---

### 3. Classes (Kelas/Halaqah)
...
### 5. Student Profiles (Profil Santri)

Data profil detail untuk santri.

| Kolom        | Tipe     | Constraint                    | Deskripsi        |
| ------------ | -------- | ----------------------------- | ---------------- |
| `id`         | UUID     | PK                            | ID unik          |
| `user_id`    | UUID     | FK → users.id, NOT NULL, UNIQ | ID pengguna      |
...
- One-to-One: Ke `users` (cascade delete)

---

### 6. Teacher Profiles (Profil Guru)

Data profil detail untuk guru tahfidz.
...
| `user_id`        | UUID     | FK → users.id, NOT NULL, UNIQ | ID pengguna        |
| `photo_url`      | TEXT     | NULLABLE                      | URL foto profil    |
| `specialization` | TEXT     | NULLABLE                      | Spesialisasi/keahlian |
| `created_at`     | DATETIME | DEFAULT now()                 | Waktu dibuat       |
| `updated_at`     | DATETIME | DEFAULT now()                 | Waktu diperbarui   |
...
### 7. Class Transfers (Perpindahan Kelas)

Catatan perpindahan santri antar kelas.

| Kolom           | Tipe     | Constraint                | Deskripsi                           |
| --------------- | -------- | ------------------------- | ----------------------------------- |
| `id`            | UUID     | PK                        | ID unik                             |
| `student_id`    | UUID     | FK → users.id, NOT NULL   | ID santri                           |
| `from_class_id` | UUID     | FK → classes.id           | ID kelas asal                       |
| `to_class_id`   | UUID     | FK → classes.id           | ID kelas tujuan                     |
| `reason`        | TEXT     | NULLABLE                  | Alasan perpindahan                  |
| `transfer_date` | TEXT     | NOT NULL                  | Tanggal perpindahan                 |
| `approved_by`   | UUID     | FK → users.id             | ID yang menyetujui                  |
...
- Many-to-One: Ke `users` (student)
- Many-to-One: Ke `classes` (from & to)
- Many-to-One: Ke `users` (approved_by)

---

### 8. Surahs (Surah Al-Quran)
...
### 10. Memorization Logs (Catatan Hafalan)
...
Data ujian hafalan dan tahfidz.

| Kolom           | Tipe     | Constraint                  | Deskripsi                          |
| --------------- | -------- | --------------------------- | ---------------------------------- |
| `id`            | UUID     | PK                          | ID unik                            |
| `name`          | TEXT     | NOT NULL                    | Nama ujian                         |
| `description`   | TEXT     | NULLABLE                    | Deskripsi                          |
| `exam_type`     | ENUM     | NOT NULL                    | mid_semester, end_semester, etc    |
| `class_id`      | UUID     | FK → classes.id             | ID kelas (opsional)                |
| `surah_id`      | INTEGER  | FK → surahs.id              | Surah spesifik (opsional)          |
| `start_surah`   | INTEGER  | FK → surahs.id              | Awal range surah                   |
| `end_surah`     | INTEGER  | FK → surahs.id              | Akhir range surah                  |
| `exam_date`     | TEXT     | NOT NULL                    | Tanggal ujian                      |
| `academic_year` | TEXT     | NOT NULL                    | Tahun ajaran (e.g. 2025/2026)      |
| `semester`      | ENUM     | NOT NULL                    | Semester (1/2)                     |
| `passing_score` | REAL     | DEFAULT 70                  | Nilai kelulusan                    |
| `max_score`     | REAL     | DEFAULT 100                 | Nilai maksimal                     |
...
- Many-to-One: Ke `classes`
- Many-to-One: Ke `surahs` (start/end)
- Many-to-One: Ke `users` (created_by)

---

### 13. Exam Results (Hasil Ujian)

Hasil ujian per santri.

| Kolom             | Tipe     | Constraint              | Deskripsi                       |
| ----------------- | -------- | ----------------------- | ------------------------------- |
| `id`              | UUID     | PK                      | ID unik                         |
| `exam_id`         | UUID     | FK → exams.id, NOT NULL | ID ujian                        |
| `student_id`      | UUID     | FK → users.id, NOT NULL | ID santri                       |
| `hafalan_score`   | REAL     | DEFAULT 0               | Nilai hafalan                   |
| `tajwid_score`    | REAL     | DEFAULT 0               | Nilai tajwid                    |
| `fashohah_score`  | REAL     | DEFAULT 0               | Nilai fashohah                  |
| `fluency_score`   | REAL     | DEFAULT 0               | Nilai kelancaran                |
| `total_score`     | REAL     | DEFAULT 0               | Nilai total                     |
| `grade`           | ENUM     | NULLABLE                | Grade (A-E)                     |
| `is_passed`       | BOOLEAN  | DEFAULT false           | Status lulus                    |
| `rank`            | INTEGER  | NULLABLE                | Peringkat di kelas              |
| `examiner_id`     | UUID     | FK → users.id           | Penguji                         |
| `notes`           | TEXT     | NULLABLE                | Catatan penguji                 |
| `feedback`        | TEXT     | NULLABLE                | Feedback untuk santri           |
| `exam_taken_at`   | DATETIME | DEFAULT now()           | Waktu pelaksanaan               |
| `created_at`      | DATETIME | DEFAULT now()           | Waktu dibuat                    |
| `updated_at`      | DATETIME | DEFAULT now()           | Waktu diperbarui                |

**Relasi:**

- Many-to-One: Ke `exams` (cascade delete)
- Many-to-One: Ke `users` (student)
- Many-to-One: Ke `users` (examiner)

---

### 14. Reports (Raport)

Raport semester santri (agregasi nilai & kehadiran).
...
| `student_id`            | UUID     | FK → users.id, NOT NULL | ID santri                       |
| `class_id`              | UUID     | FK → classes.id         | ID kelas                        |
| `academic_year`         | TEXT     | NOT NULL                | Tahun ajaran                    |
| `semester`              | ENUM     | NOT NULL                | Semester                        |
| `total_sessions`        | INTEGER  | DEFAULT 0               | Total sesi                      |
| `present_count`         | INTEGER  | DEFAULT 0               | Total hadir                     |
| `absent_count`          | INTEGER  | DEFAULT 0               | Total alfa                      |
| `sick_count`            | INTEGER  | DEFAULT 0               | Total sakit                     |
| `attendance_percentage` | REAL     | DEFAULT 0               | Persentase kehadiran            |
| `total_ayahs_memorized` | INTEGER  | DEFAULT 0               | Total ayat dihafal              |
| `current_surah`         | INTEGER  | FK → surahs.id          | Surah terakhir                  |
| `avg_total_score`       | REAL     | DEFAULT 0               | Rata-rata nilai harian          |
| `mid_semester_score`    | REAL     | NULLABLE                | Nilai UTS                       |
| `final_score`           | REAL     | DEFAULT 0               | Nilai Akhir                     |
| `final_grade`           | ENUM     | NULLABLE                | Grade Akhir                     |
| `class_rank`            | INTEGER  | NULLABLE                | Peringkat                       |
| `status`                | ENUM     | DEFAULT 'draft'         | draft, published, archived      |
| `teacher_notes`         | TEXT     | NULLABLE                | Catatan guru                    |
...
### 16. School Profiles (Profil Sekolah)

Data profil lembaga atau sekolah.

| Kolom        | Tipe     | Constraint                    | Deskripsi        |
| ------------ | -------- | ----------------------------- | ---------------- |
| `id`         | UUID     | PK                            | ID unik          |
| `name`       | TEXT     | NOT NULL                      | Nama sekolah     |
| `address`    | TEXT     | NULLABLE                      | Alamat           |
| `phone`      | TEXT     | NULLABLE                      | Telepon          |
| `email`      | TEXT     | NULLABLE                      | Email            |
| `website`    | TEXT     | NULLABLE                      | Website          |
| `logo_url`   | TEXT     | NULLABLE                      | URL Logo         |
| `created_at` | DATETIME | DEFAULT now()                 | Waktu dibuat     |
| `updated_at` | DATETIME | DEFAULT now()                 | Waktu diperbarui |
...
_Dokumentasi ini dihasilkan untuk Tahfidz Bootcamp API v1.2.0_
