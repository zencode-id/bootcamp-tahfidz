...
import { createClassSchema, updateClassSchema, addClassMemberSchema, } from "../validators/index.js";
import { authMiddleware, adminOnly, teacherOrAdmin, } from "../middleware/auth.js";
...
classRoutes.get("/", async (c) => {
    ...
    if (user.role === "admin") {
        ...
        allClasses = rawClasses.filter((cls) => classIds.includes(cls.id));
    }
    // Get teacher names and member counts
    const classesWithDetails = await Promise.all(allClasses.map(async (cls) => {
        let teacher = null;
        if (cls.teacherId) {
...
// ============================================
classRoutes.post("/", adminOnly, zValidator("json", createClassSchema), async (c) => {
...
classRoutes.get("/:id", async (c) => {
    ...
    const studentsWithDetails = await Promise.all(members.map(async (member) => {
...
// ============================================
classRoutes.put("/:id", adminOnly, zValidator("json", updateClassSchema), async (c) => {
    const { id } = c.req.param();
    const body = c.req.valid("json");
...
// ============================================
classRoutes.patch("/:id", adminOnly, zValidator("json", updateClassSchema), async (c) => {
    const { id } = c.req.param();
    const body = c.req.valid("json");
    const existing = await db.classes.findFirst({
        id: id,
...
// ============================================
classRoutes.post("/:id/members", teacherOrAdmin, zValidator("json", addClassMemberSchema.pick({ studentId: true })), async (c) => {
...
classRoutes.post("/cleanup-inactive", adminOnly, async (c) => {
    ...
    const membershipsToRemove = allMembers.filter((m) => inactiveUserIds.includes(m.studentId));
    let removedCount = 0;
