...
import { authMiddleware, canAccessStudent, } from "../middleware/auth.js";
...
stats.get("/progress/:studentId", async (c) => {
    ...
    const ziyadahLogs = allLogs.filter((l) => l.type === "ziyadah");
    ...
    for (let juz = 1; juz <= 30; juz++) {
        ...
        const relevantLogs = ziyadahLogs.filter((log) => {
        ...
        const ayahsMemorizedRaw = relevantLogs.reduce((sum, log) => sum + (Number(log.endAyah) - Number(log.startAyah) + 1), 0) / 3;
    ...
    if (logIds.length > 0) {
        ...
        const studentAssessments = allAssessments.filter((a) => logIds.includes(a.logId));
        if (studentAssessments.length > 0) {
            avgScores = {
                tajwid: studentAssessments.reduce((sum, a) => sum + Number(a.tajwidScore || 0), 0) /
                    studentAssessments.length,
                fashohah: studentAssessments.reduce((sum, a) => sum + Number(a.fashohahScore || 0), 0) /
                    studentAssessments.length,
                fluency: studentAssessments.reduce((sum, a) => sum + Number(a.fluencyScore || 0), 0) /
                    studentAssessments.length,
                total: studentAssessments.reduce((sum, a) => sum + Number(a.totalScore || 0), 0) /
                    studentAssessments.length,
    ...
    // Get recent activity
    const recentLogs = [...allLogs].sort((a, b) => b.sessionDate.localeCompare(a.sessionDate)).slice(0, 10);
    // Get surah progress
    const surahProgressMap = new Map();
    ziyadahLogs.forEach((log) => {
        const sId = Number(log.surahId);
...
stats.get("/attendance/:studentId", zValidator("query", attendanceStatsSchema), async (c) => {
    const { studentId } = c.req.param();
    ...
    records.forEach((r) => {
        ...
        if (!sessionTypeStats[key])
            sessionTypeStats[key] = { sessionType: r.sessionType, status: r.status, count: 0 };
...
stats.get("/class/:classId", async (c) => {
    ...
    const recentAtt = allAtt.filter((a) => a.date >= thirtyDaysAgo);
    const attendanceStatsMap = {};
    recentAtt.forEach((a) => {
        if (!attendanceStatsMap[a.status])
            attendanceStatsMap[a.status] = { status: a.status, count: 0 };
    ...
    const memorizationStatsMap = {};
    allMem.forEach((l) => {
        if (!memorizationStatsMap[l.type])
            memorizationStatsMap[l.type] = { type: l.type, totalAyahs: 0, sessions: 0 };
...
stats.get("/leaderboard", zValidator("query", leaderboardSchema), async (c) => {
    ...
    if (query.type === "ayahs") {
        ...
        const ziyadah = allLogs.filter((l) => l.type === "ziyadah");
        // Group by student
        const studentCounts = {};
        ziyadah.forEach((l) => {
            if (!studentCounts[l.studentId])
                studentCounts[l.studentId] = 0;
        ...
        memLogs.forEach((l) => logMap.set(l.id, l.studentId));
        const studentScores = {};
        assessments.forEach((a) => {
            const sId = logMap.get(a.logId);
            if (sId) {
                if (!studentScores[sId])
                    studentScores[sId] = { sum: 0, count: 0 };
        ...
        const recent = attendance.filter((a) => a.date >= thirtyDaysAgo);
        const studentAtt = {};
        recent.forEach((a) => {
            if (!studentAtt[a.studentId])
                studentAtt[a.studentId] = { present: 0, total: 0 };
            studentAtt[a.studentId].total++;
            if (a.status === 'present' || a.status === 'late')
                studentAtt[a.studentId].present++;
        });
