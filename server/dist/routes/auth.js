...
import { loginSchema, registerSchema, updateUserSchema, } from "../validators/index.js";
...
auth.post("/verify-otp", async (c) => {
    ...
    const existingOtp = otps.sort((a, b) => {
...
async function updateTeacherProfile(userId, body) {
...
auth.get("/users", authMiddleware, adminOnly, async (c) => {
    ...
    let allUsers = await db.users.findMany({}, 1000);
    // 2. Filter in Memory
    if (role) {
    ...
    if (search) {
        ...
        allUsers = allUsers.filter(u => (u.name && u.name.toLowerCase().includes(searchLower)) ||
...
auth.put("/users/:id", authMiddleware, adminOnly, zValidator("json", updateUserSchema), async (c) => {
    const { id } = c.req.param();
    const body = c.req.valid("json");
...
auth.patch("/users/:id", authMiddleware, adminOnly, zValidator("json", updateUserSchema), async (c) => {
    const { id } = c.req.param();
    const body = c.req.valid("json");
    // Check if user exists
    const existingUser = await db.users.findFirst({
