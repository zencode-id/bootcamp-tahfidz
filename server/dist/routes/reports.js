...
import { authMiddleware, teacherOrAdmin, adminOnly, getAccessibleStudentIds, } from "../middleware/auth.js";
...
reportRoutes.get("/", async (c) => {
    ...
    reportList.sort((a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime());
    // Enrich with student names
    // Potentially slow if many reports. Limit concurrency.
    const enrichedReports = await Promise.all(reportList.map(async (report) => {
...
reportRoutes.get("/:id", async (c) => {
    ...
    if (report.currentSurah) {
        ...
        currentSurahInfo = surahs.find((s) => s.id == report.currentSurah);
    }
...
reportRoutes.post("/", teacherOrAdmin, zValidator("json", createReportSchema), async (c) => {
    const auth = c.get("auth");
    ...
    const existing = existingReports.find((r) => r.academicYear === data.academicYear &&
        r.semester === data.semester);
    if (existing) {
        throw new HTTPException(400, {
            message: "Report already exists for this student, academic year, and semester",
        });
    }
...
reportRoutes.post("/generate", teacherOrAdmin, zValidator("json", generateReportSchema), async (c) => {
    const auth = c.get("auth");
    const { classId, academicYear, semester, studentIds: providedStudentIds, } = c.req.valid("json");
    ...
    if (providedStudentIds && providedStudentIds.length > 0) {
        ...
        studentIds = allStudents.filter((u) => u.role === "student").map((s) => s.id);
    }
    ...
    for (const studentId of studentIds) {
        try {
            ...
            const existing = existingReports.find((r) => r.academicYear === academicYear &&
                r.semester === semester);
            if (existing) {
            ...
            const sickCount = attendanceRecords.filter((a) => a.status === "sick").length;
            ...
            const memLogs = allMemLogs.filter((l) => l.type === "ziyadah" &&
                l.sessionDate >= startDate &&
                l.sessionDate <= endDate);
            const totalNewAyahs = memLogs.reduce((sum, log) => sum + (log.endAyah - log.startAyah + 1), 0);
            // Get total memorized (all time Ziyadah)
            const allZiyadahLogs = allMemLogs.filter((l) => l.type === "ziyadah");
            const totalAyahsMemorized = allZiyadahLogs.reduce((sum, log) => sum + (log.endAyah - log.startAyah + 1), 0);
            // Get murojaah sessions
            const murojaahLogs = allMemLogs.filter((l) => l.type === "murojaah" &&
                l.sessionDate >= startDate &&
                l.sessionDate <= endDate);
            // Get latest surah/ayah (from all logs)
            const sortedLogs = allMemLogs.sort((a, b) => b.sessionDate.localeCompare(a.sessionDate));
            ...
            const semesterAssessments = allAssessments.filter((a) => logIds.includes(a.logId));
            const avgTajwidScore = semesterAssessments.length > 0
                ? semesterAssessments.reduce((sum, a) => sum + (Number(a.tajwidScore) || 0), 0) /
                    semesterAssessments.length
                : 0;
            const avgFashohahScore = semesterAssessments.length > 0
                ? semesterAssessments.reduce((sum, a) => sum + (Number(a.fashohahScore) || 0), 0) /
                    semesterAssessments.length
                : 0;
            const avgFluencyScore = semesterAssessments.length > 0
                ? semesterAssessments.reduce((sum, a) => sum + (Number(a.fluencyScore) || 0), 0) /
                    semesterAssessments.length
                : 0;
            const avgTotalScore = (avgTajwidScore + avgFashohahScore + avgFluencyScore) / 3;
            // Get exam scores
            // Fetch all exams for this semester
            const allExams = await db.exams.findMany({});
            const semesterExams = allExams.filter((e) => e.academicYear === academicYear &&
                e.semester === semester);
            ...
            for (const exam of semesterExams) {
                const result = studentExamResults.find((r) => r.examId === exam.id);
                if (result) {
...
reportRoutes.put("/:id", teacherOrAdmin, zValidator("json", updateReportSchema), async (c) => {
    const reportId = c.req.param("id");
    const data = c.req.valid("json");
...
reportRoutes.post("/publish-bulk", adminOnly, zValidator("json", z.object({ reportIds: z.array(z.string().uuid()) })), async (c) => {
    const auth = c.get("auth");
