...
import { bulkTahfidzSchema, memorizationLogItemSchema, assessmentItemSchema, updateMemorizationLogSchema, updateAssessmentSchema, paginationSchema, dateRangeSchema, } from "../validators/index.js";
...
// ============================================
tahfidz.post("/", teacherOrAdmin, zValidator("json", bulkTahfidzSchema), async (c) => {
    ...
    for (const assessment of assessmentItems) {
        try {
            ...
            const totalScore = (assessment.tajwidScore +
...
// ============================================
tahfidz.get("/logs", zValidator("query", memorizationQuerySchema), async (c) => {
    ...
    if (accessibleIds !== "all") {
        logs = logs.filter((l) => accessibleIds.includes(l.studentId));
    }
    if (query.type)
        logs = logs.filter((l) => l.type === query.type);
    if (query.surahId)
        logs = logs.filter((l) => l.surahId == query.surahId);
    if (query.startDate)
        logs = logs.filter((l) => l.sessionDate >= query.startDate);
    if (query.endDate)
        logs = logs.filter((l) => l.sessionDate <= query.endDate);
...
tahfidz.get("/logs/:id", async (c) => {
    ...
    const assessment = allAssessments.find((a) => a.logId === id) || null;
    return c.json({
        success: true,
        data: {
            ...log,
...
// ============================================
tahfidz.put("/logs/:id", teacherOrAdmin, zValidator("json", memorizationLogItemSchema), async (c) => {
    const { id } = c.req.param();
...
// ============================================
tahfidz.patch("/logs/:id", teacherOrAdmin, zValidator("json", updateMemorizationLogSchema), async (c) => {
    const { id } = c.req.param();
    const body = c.req.valid("json");
    const existing = await db.memorizationLogs.findFirst({ id: id });
    if (!existing) {
...
tahfidz.delete("/logs/:id", teacherOrAdmin, async (c) => {
    ...
    const allAssessments = await db.assessments.findMany({});
    const assessment = allAssessments.find((a) => a.logId === id);
    if (assessment) {
...
tahfidz.post("/assessments", teacherOrAdmin, zValidator("json", assessmentItemSchema), async (c) => {
    const body = c.req.valid("json");
    const { user } = c.get("auth");
    // Check if log exists
    ...
    const totalScore = (body.tajwidScore + body.fashohahScore + body.fluencyScore) / 3;
...
tahfidz.put("/assessments/:id", teacherOrAdmin, zValidator("json", assessmentItemSchema), async (c) => {
    const { id } = c.req.param();
    const body = c.req.valid("json");
...
tahfidz.patch("/assessments/:id", teacherOrAdmin, zValidator("json", updateAssessmentSchema), async (c) => {
    const { id } = c.req.param();
    const body = c.req.valid("json");
    const existing = await db.assessments.findFirst({ id: id });
    if (!existing) {
...
tahfidz.get("/surahs", async (c) => {
    ...
    // Sort by id
    allSurahs.sort((a, b) => Number(a.id) - Number(b.id));
    return c.json({
        success: true,
        data: allSurahs,
    });
