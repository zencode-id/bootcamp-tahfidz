...
import { authMiddleware, teacherOrAdmin, adminOnly, } from "../middleware/auth.js";
...
examRoutes.post("/", teacherOrAdmin, zValidator("json", createExamSchema), async (c) => {
    const auth = c.get("auth");
...
examRoutes.put("/:id", teacherOrAdmin, zValidator("json", createExamSchema.partial()), async (c) => {
    const examId = c.req.param("id");
    const data = c.req.valid("json");
    // Check exist
...
examRoutes.post("/:id/results", teacherOrAdmin, zValidator("json", examResultSchema.omit({ examId: true })), async (c) => {
    const examId = c.req.param("id");
    const auth = c.get("auth");
    const data = c.req.valid("json");
    // Check exam exists
    ...
    const existing = results.find((r) => r.studentId === data.studentId);
    if (existing) {
    ...
    if (data.makhorijulHurufScore)
        scores.push(data.makhorijulHurufScore);
    if (data.tartilScore)
        scores.push(data.tartilScore);
...
examRoutes.post("/:id/results/bulk", teacherOrAdmin, zValidator("json", z.object({ results: z.array(examResultSchema.omit({ examId: true })) })), async (c) => {
    const examId = c.req.param("id");
    const auth = c.get("auth");
    ...
    existingResults.forEach((r) => existingMap.set(r.studentId, r));
    ...
    for (const item of resultItems) {
        try {
            ...
            if (item.makhorijulHurufScore)
                scores.push(item.makhorijulHurufScore);
            if (item.tartilScore)
                scores.push(item.tartilScore);
            const totalScore = scores.reduce((a, b) => a + b, 0) / scores.length;
            // Determine grade
            let grade;
            if (totalScore >= 90)
...
examRoutes.put("/:id/results/:studentId", teacherOrAdmin, zValidator("json", examResultSchema.omit({ examId: true, studentId: true }).partial()), async (c) => {
    const examId = c.req.param("id");
    const studentId = c.req.param("studentId");
    const auth = c.get("auth");
    const data = c.req.valid("json");
    ...
    if (data.makhorijulHurufScore ?? existing.makhorijulHurufScore)
        scores.push((data.makhorijulHurufScore ?? existing.makhorijulHurufScore));
    if (data.tartilScore ?? existing.tartilScore)
        scores.push((data.tartilScore ?? existing.tartilScore));
    const totalScore = scores.reduce((a, b) => a + b, 0) / scores.length;
    let grade;
    if (totalScore >= 90)
        grade = "A";
